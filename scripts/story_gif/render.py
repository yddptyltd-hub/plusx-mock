...
async def main():
    async with async_playwright() as p:
        ...
        # Load the HTML
        file_path = f"http://localhost:{PORT}/scripts/story_gif/scene.html"
...
# Wait for fonts to load
    ...
    subprocess.run(
        'ffmpeg -y -framerate 30 -i scripts/story_gif/frame_%04d.png -i scripts/story_gif/palette.png -lavfi "fps=20,scale=1080:-1:flags=lanczos[x];[x][1:v]paletteuse" public/share/lpx-story.gif',
        shell=True, check=True
    )

    print("Optimizing GIF...")
    subprocess.run(
        'gifsicle -O3 --lossy=80 public/share/lpx-story.gif -o public/share/lpx-story.gif',
        shell=True, check=True
    )

    print("Composing MP4...")
    subprocess.run(
        'ffmpeg -y -framerate 30 -i scripts/story_gif/frame_%04d.png -c:v libx264 -preset slow -crf 20 -pix_fmt yuv420p -movflags +faststart public/share/lpx-story.mp4',
        shell=True, check=True
    )

    print("Cleanup...")
    subprocess.run('rm scripts/story_gif/frame_*.png scripts/story_gif/palette.png', shell=True)

    # Verification
    gif_size = os.path.getsize('public/share/lpx-story.gif')
